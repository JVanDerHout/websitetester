---
title: Where
nav:
  order: 2
  tooltip: Coming to Pescadero
permalink: /location/
---
# {% include icon.html icon="fa-solid fa-location-dot" %}The Coast
All Sunday events will be held at <a href="https://www.pieranch.org">Pie Ranch</a> in Pescadero, CA.
Pie Ranch is located at 2080 CA-1, Pescadero, CA 94060.
Here's a <a href="https://www.google.com/maps/place/Pie+Ranch/@37.1321082,-122.3163959,17z/data=!4m6!3m5!1s0x808e55caafeb2abb:0xcf813edfe879e0f3!8m2!3d37.132104!4d-122.313821!16s%2Fg%2F1thw6zd3?entry=ttu&g_ep=EgoyMDI2MDYyOS4wIKXMDSoASAFQAw%3D%3D"> Google Maps Link </a>.
## {% include icon.html icon="fa-solid fa-location-dot" %}Getting to Pie Ranch
Pescadero sits along the San Mateo coast, about an hour south of San Francisco, and there isn't a direct public transit option, so plan to drive yourself (or even better, carpool). <br>
<br>
**Flying in:** <br>
&emsp;• <strong>San Francisco (SFO)</strong>, the closest major airport, about 1 hour from Pescadero if there is no traffic. Also the airport with the most direct flight options if you're coming from out of town. <br>
&emsp;• <strong>San Jose (SJC)</strong>, about 1 hour from Pescadero if there is no traffic, and can be a good alternative if you're planning to stay in Santa Cruz. <br>
&emsp;• <strong>Oakland (OAK)</strong>, about 1 hour 15 minutes from Pescadero if there is no traffic (but let's be real, that is very unlikely). Sometimes has better fares than SFO. <br>
<br>
**Driving in from around the Bay Area:** <br>
&emsp;• From <strong>San Francisco</strong>, about 1 hour and 15 minutes, or more if coming from downtown. <br>
&emsp;• From the <strong>East Bay</strong>, about 1 hour and 30 minutes or maybe more depending on traffic. <br>
&emsp;• From <strong>Half Moon Bay</strong>, about 30 minutes south via Highway 1. <br>
&emsp;• From <strong>Santa Cruz</strong>, about 30 minutes north via Highway 1. <br>

<br>
A note on the drive: the roads out to Pescadero can have limited cell service in stretches, so we recommend downloading your route ahead of time. <br>
<br>
<!-- consider if we want to do some kind of land acknowledgement, http://www.mobileranger.com/santacruz/ohlone-coastanoan-or-awaswas-correcting-histories-misnomers/ -->
{%
  include button.html
  type="address"
  tooltip="Pie Ranch"
  link="https://www.google.com/maps/place/Pie+Ranch/@37.1321082,-122.3163959,17z/data=!4m6!3m5!1s0x808e55caafeb2abb:0xcf813edfe879e0f3!8m2!3d37.132104!4d-122.313821!16s%2Fg%2F1thw6zd3?entry=ttu&g_ep=EgoyMDI2MDYyOS4wIKXMDSoASAFQAw%3D%3D"
%}
{% include section.html %}
{% include section.html dark=true %}
